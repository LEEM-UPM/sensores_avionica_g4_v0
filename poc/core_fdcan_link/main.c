...
double pads_output;
int64_t pads_pressure_encoded;
struct ASPID_FDCAN_NETWORK_imu_out_t imu_out;
struct ASPID_FDCAN_NETWORK_gnss_out_t gnss_out;
struct ASPID_FDCAN_NETWORK_bar_out_t bar_out;
...
static void aspid_fdcan_newtork_init();

// ----- PREPROCESSED -----
FDCAN_FilterTypeDef Airbrake_filter;
FDCAN_TxHeaderTypeDef bar_header;
...
int main(void)
{
  ...
  while (1){
    if(fifo_event){
        ...
        mempcy(&pads_output,&pressure_avg[MAX_GROUPS],sizeof(pads_output));
        pads_pressure_encoded = ASPID_FDCAN_NETWORK_bar_out_bar_out_sig484_encode(pads_output);
        // TODO configurar filtro
        bar_out.bar_out_sig484 = pads_pressure_encoded;
        HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1,&bar_header,&bar_out);
    }
  }
}


static void aspid_fdcan_network_init(){
    ASPID_FDCAN_NETWORK_bar_out_init(&bar_out);
    ASPID_FDCAN_NETWORK_gnss_out_init(&gnss_out);
    ASPID_FDCAN_NETWORK_imu_out_init(&imu_out);
    
    // configure all headers
    bar_header.Identifier = ASPID_FDCAN_NETWORK_BAR_OUT_FRAME_ID;
}
