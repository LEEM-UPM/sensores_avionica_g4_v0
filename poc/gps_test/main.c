...
const uint8_t UBX_CFG_RATE_5HZ[] = {
    ...
    0x01, 0x00, 0x21, 0x30, 0xC8, 0x00, 0xF3, 0xAC // Checksum calculado (alg de
                                                   // fletcher)
};
...
int main() {
  ...
  while (1) {
    ...
    // Access g_latitud, g_longitud, g_satelites_activos as needed
    char long_buffer[50];
    int len = snprintf(long_buffer, sizeof(long_buffer), "Longitud: %.6f\r\n", (double)g_longitud);

    HAL_UART_Transmit(&huart2, (uint8_t *)long_buffer, len, HAL_MAX_DELAY);
    HAL_Delay(1000);
  }
}
...
void M10S_Init_Configuration(UART_HandleTypeDef *huart) {
  HAL_Delay(500);
  HAL_UART_Transmit(huart, (uint8_t *)UBX_CFG_RATE_5HZ,
                    sizeof(UBX_CFG_RATE_5HZ), 100);
  HAL_Delay(100);
}
    ...
    if (rx_byte == '\n') {
      ...
      if (minmea_sentence_id(line_buffer, false) == MINMEA_SENTENCE_RMC) {
        ...
        if (minmea_parse_rmc(&frame, line_buffer)) {
          ...
          g_longitud = minmea_tocoord(&frame.longitude);
