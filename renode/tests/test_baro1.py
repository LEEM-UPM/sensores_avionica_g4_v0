...
baro1 = machine["sysbus.i2c1.baro1"]
    ...
    while True:
        if not machine.IsPaused:
            try:
                ...
                # set properties in C#
                baro1.Pressure = System.Convert.ToDecimal(pressure)
                baro1.Temperature = System.Convert.ToDecimal(temperature)
                
                print("Baro1 updated -> Pressure: %.2f Pa, Temperature: %.2f C" % (pressure, temperature))
            except Exception as e:
                print("Baro1 test error: %s" % str(e))
...
t.IsBackground = True
t.Start()
print("Baro1 random data simulation started")
